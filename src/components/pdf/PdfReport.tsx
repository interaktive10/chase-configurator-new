...
export function PdfReport() {
  ...
  const L = config.l; // Length -> X axis
  const W = config.w; // Width -> Y axis
  ...
  const renderHoles = () => {
    ...
    const getSpacing = (n: number) => {
      if (n === 2) {
        const defaultSpacing = L / 4;
        const minSpacing = (config.collarA.dia / 2) + (config.collarB.dia / 2) + minGap;
        return Math.max(defaultSpacing, minSpacing);
      }
      if (n === 3) {
        const defaultSpacing = L / 3;
        const minAB = (config.collarA.dia / 2) + (config.collarB.dia / 2) + minGap;
        const minBC = (config.collarB.dia / 2) + (config.collarC.dia / 2) + minGap;
        return { spAB: Math.max(defaultSpacing, minAB), spBC: Math.max(defaultSpacing, minBC) };
      }
      return 0;
    };

    const addHole = (id: 'A' | 'B' | 'C', collar: any, defaultX: number) => {
      let xInches = defaultX;
      let yInches = W / 2;

      if (!collar.centered) {
        xInches = collar.offset2; // From Left
        yInches = W - collar.offset1; // From Top (since offset1 is from Bottom)
      }

      const hx = rectX + xInches * scale;
      const hy = rectY + yInches * scale;
      const hr = (collar.dia / 2) * scale;
      
      holes.push({ id, hx, hy, hr, collar, xInches, yInches });
    };

    if (config.holes >= 1) {
      if (config.holes === 1) addHole('A', config.collarA, L / 2);
      if (config.holes === 2) {
        const sp = getSpacing(2) as number;
        addHole('A', config.collarA, L / 2 - sp);
        addHole('B', config.collarB, L / 2 + sp);
      }
      if (config.holes === 3) {
        const { spAB, spBC } = getSpacing(3) as { spAB: number, spBC: number };
        addHole('A', config.collarA, L / 2 - spAB);
        addHole('B', config.collarB, L / 2);
        addHole('C', config.collarC, L / 2 + spBC);
      }
    }

    return holes.map(({ id, hx, hy, hr, collar, xInches, yInches }) => {
      const showArrows = !collar.centered;

      return (
        <g key={id} data-x={xInches} data-y={yInches}>
          {/* Dimension Arrows if not centered */}
          {showArrows && (
            <>
               {/* Arrow from Left Edge */}
               <line x1={rectX} y1={hy} x2={hx - hr - 2} y2={hy} stroke="#888" strokeWidth="1" markerEnd="url(#arrow)" markerStart="url(#arrow)" />
               <text x={rectX + (xInches * scale) / 2} y={hy - 4} fontSize="10" fill="#666" textAnchor="middle">{id}4: {xInches}"</text>
               
               {/* Arrow from Bottom Edge */}
               <line x1={hx} y1={rectY + drawH} x2={hx} y2={hy + hr + 2} stroke="#888" strokeWidth="1" markerEnd="url(#arrow)" markerStart="url(#arrow)" />
               <text x={hx + 4} y={rectY + drawH - ((collar.offset1 * scale) / 2)} fontSize="10" fill="#666" alignmentBaseline="middle">{id}3: {collar.offset1}"</text>
            </>
          )}

          <circle cx={hx} cy={hy} r={hr} fill="white" stroke="black" strokeWidth="2" />
                  ...
                  <tbody>
                    {config.holes >= 1 && (
                      <tr>
                        ...
                        <td style={{ padding: '4px 0' }}>{config.collarA.dia}" Dia, {config.collarA.height}" Height {!config.collarA.centered && <span style={{ color: '#666', fontSize: '12px' }}>- Offset (A4: {config.collarA.offset2}", A3: {config.collarA.offset1}")</span>}</td>
                      </tr>
                    )}
                    {config.holes >= 2 && (
                      <tr>
                        <td style={{ padding: '4px 0', fontWeight: 'bold' }}>Hole B</td>
                        <td style={{ padding: '4px 0' }}>{config.collarB.dia}" Dia, {config.collarB.height}" Height {!config.collarB.centered && <span style={{ color: '#666', fontSize: '12px' }}>- Offset (B4: {config.collarB.offset2}", B3: {config.collarB.offset1}")</span>}</td>
                      </tr>
                    )}
                    {config.holes === 3 && (
                      <tr>
                        <td style={{ padding: '4px 0', fontWeight: 'bold' }}>Hole C</td>
                        <td style={{ padding: '4px 0' }}>{config.collarC.dia}" Dia, {config.collarC.height}" Height {!config.collarC.centered && <span style={{ color: '#666', fontSize: '12px' }}>- Offset (C4: {config.collarC.offset2}", C3: {config.collarC.offset1}")</span>}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
